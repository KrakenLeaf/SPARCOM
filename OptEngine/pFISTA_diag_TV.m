function [ x, fun_all, lambda_stack ] = pFISTA_diag_TV( Y, H, Params )
% PFISTA_DIAG - Performs the Fast Proximal Gradient method on the problem
%
%              min_{x} \lambda*TV( x ) + 0.5|| Y - AXA^H ||_F^2
%
...
for kk = 1:Params.IterMax
    ...
    % -------------------------------     TV DENOISING     --------------------------------------- %
    % Prox calculation part: Denoising, based on Amir Beck's paper. This code snippet is taken from
    % the function "deblur_tv_fista.m". TV denoising includes box constraints such as non-negativity.
    ...
    % Upper bound is Inf.
    parsin.MAXITER = Params.DenoiseIter;
    parsin.epsilon = 1e-5;
    parsin.print   = 0;
    parsin.tv      = Params.TV;  % 'iso': Isotropic TV norm, 'l1': Anisotropic TV norm
    parsin.fig     = 0;
    if kk == 1
        [Z_iter,iter,fun_denoise,P] = denoise_bound_init(reshape(g, sqrt(N), sqrt(N)), lambda/L, lb, Inf, [] ,parsin);           % First iteration
    else
        [Z_iter,iter,fun_denoise,P] = denoise_bound_init(reshape(g, sqrt(N), sqrt(N)), lambda/L, lb, Inf, P, parsin);            % Other iterations with warm start
    end
    
    % Reshaping the result to be a vector again
    x_prev = x;
    ...
    if Params.mon == 0
        ...
        t = tlv(Z_iter, Params.TV);
        %fun_val = 0.5*norm(Y - ,'fro')^2 + lambda*t;    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 2*lambda
        if kk > 1
